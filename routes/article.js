...
var searchValue = '';  //这里保存着模糊查询的值 // 这个是个bug，这特么的刚发现
...
// 某一个用户查询自己的页面的配置
router.get('/listself-:page.html', (req, res) => {
    let loginCookie = req.cookies.login;
    if (!loginCookie) {
        return res.render('404');
    }
    let userid = req.cookies.login.id;
    let page = (req.params['page'] - 1) * 7;

    sql('select * from article2,bloguser where article2.userid = ? and article2.userid = bloguser.bloguserid order by articleid desc limit ?,7', [userid, page], (err, data) => {
        if (err) throw err;
